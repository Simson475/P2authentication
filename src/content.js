    ...
    if (response.token === null) {  // If null, not currently logged in
        ...
        window.onload = () => { // Waits for the page to be fully loaded to execute the autofill process.
            chrome.runtime.sendMessage({ sendURL: domainURL }, (response) => { // Sends a message to background containing the current URL.
                if (response.fetchAnswer === undefined) { // Unexpected error handling.
                    //TODO ERROR HANDLING
                    console.log(response.fetchAnswer);
                } else {
                    autofill(response.fetchAnswer.username, response.fetchAnswer.password); // Autofills the web page, with the saved username and password.
                }
            })
        }
    }
...
chrome.runtime.onMessage.addListener(async function(request, sender, sendResponse) { // Listens for a messages.
    if (request.autofillPassword !== undefined) {   // Checks if message contains the field 'autofillPassword'
        ...
        password.value = request.autofillPassword;            // inserts the user's password into the password field that has been found by findPasswordFieldInForm      
        password.innerHTML = request.autofillPassword;        // Overwrites the innerHTML of the password field to be the password.
...
function findPasswordFieldInForm(form) {
    ...
    for (elem of inputFields) { 
        if (elem.type === "password") return elem         // If an element with the type "password" is found then, it returns that element. 
    }
    return false; // if no elements with the type password can be found in the form it returns false
} 