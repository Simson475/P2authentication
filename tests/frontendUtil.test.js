...
test("hashCode should always give the same output with same input", () => {
    const firstResult = hashCode("ELON A Æ X-12")
    const secondResult = hashCode("ELON A Æ X-12")
...
test("hashCode should always give the same output with same input now tested with really long names", () => {
    const firstResult = hashCode("ELON A Æ X-12 og så bare en masse bullshit bagefter12")
    const secondResult = hashCode("ELON A Æ X-12 og så bare en masse bullshit bagefter")
    expect(firstResult).not.toBe(secondResult)
})
...
test("hashCode should with two hashes with different names in general not give the same outcome now tested with the same ending, but with different beginning", () => {
    const firstResult = hashCode("DET HER ER BULLSHIT ÅBENBART, 123456789 ELON A Æ X-12")
    const secondResult = hashCode("DET HER, ER OGSÅ BULLSHIT ÅBENBART 987654321 ELON A Æ X-12")
    expect(firstResult).not.toBe(secondResult)
})
